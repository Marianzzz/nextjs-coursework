    ...
    >
      <ResizablePanel defaultSize={50} minSize={40} className="lg:block hidden">
        {first && (
          <Link
            href={`/news/${first.id}`}
            className="block relative w-full h-full min-h-[500px] overflow-hidden group"
          >
            {first.imageUrl && (
              <Image
                src={first.imageUrl}
            ...
            <div className="absolute inset-0 bg-black/50 flex flex-col justify-end p-6 transition-all duration-300 group-hover:bg-black/60">
              <h2 className="text-white font-bold text-3xl leading-tight">{first.title}</h2>
              <p className="text-white text-sm mt-2">
                {format(new Date(first.publishedAt), "dd MMMM yyyy", { locale: uk })}
              </p>
              <p className="text-white font-bold text-sm mt-2">{first.discipline}</p>
            </div>
          </Link>
        )}
      </ResizablePanel>
            ...
            >
              ...
              <div className="absolute inset-0 bg-black/50 flex flex-col justify-end p-3 transition-all duration-300 group-hover:bg-black/60">
                ...
                <p className="text-white text-xs mt-1">
                  {format(new Date(item.publishedAt), "dd MMMM yyyy", { locale: uk })}
                </p>
                <p className="text-white font-bold text-xs mt-1">{item.discipline}</p>
              </div>
            </Link>
          ))}
          {first && (
            ...
            >
              ...
              <div className="absolute inset-0 bg-black/50 flex flex-col justify-end p-3 transition-all duration-300 group-hover:bg-black/60">
                <h3 className="text-white font-semibold text-sm leading-tight">{first.title}</h3>
                <p className="text-white text-xs mt-1">
                  {format(new Date(first.publishedAt), "dd MMMM yyyy", { locale: uk })}
                </p>
                <p className="text-white font-bold text-xs mt-1">{first.discipline}</p>
              </div>
            </Link>
          )}
        </div>
